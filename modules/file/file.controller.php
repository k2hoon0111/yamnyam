        ...
        function insertFile($file_info, $module_srl, $upload_target_srl, $download_count = 0, $manual_insert = false) {
            ...
            */
			...
			if(preg_match("/\.(jpg|jpeg)$/i", $file)) {
		        ...
		        $overlay_img=ImageCreateFromPNG("watermark.png");
		        $overlay_img_x = imageSX($overlay_img); //오버레이 이미지 사이즈 X값
		        $overlay_img_y = imageSY($overlay_img); //오버레이 이미지 사이즈 Y값
		
		        $offsetX = ($max_width - ImageSX($overlay_img))/2; //왼쪽 아래에서 3픽셀만큼 떨어진곳에 로고를 새깁니다
		        $offsetY = ($max_height - ImageSY($overlay_img))/2;
