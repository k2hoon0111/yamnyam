<?php
    ...
    class memberController extends member {
        ...
        function procMemberCheckValue() {
            ...
            switch($name) {
                case 'user_id' :
						...
						if(!ereg("[[:alnum:]+]{3,20}",$value)) return new Object(0, 'msg_unauth_user_id');

						return new Object(0, 'allowed_user_id');
                    break;
                case 'nick_name' :
                        // 중복 검사
                ...
                case 'email_address' :
						...
						if($group == '0'){
							if(!ereg("(^[_0-9a-zA-Z-]+(\.[_0-9a-zA-Z-]+)*@[0-9a-zA-Z-]+(\.[0-9a-zA-Z-]+)*$)", $value)) return new Object(0, 'msg_unauth_email_address2');
							if($member_srl && $logged_info->member_srl != $member_srl ) return new Object(0,'msg_exists_email_address');
						} else {
	                        // 중복 검사
							if(!ereg("(^[_0-9a-zA-Z-]+(\.[_0-9a-zA-Z-]+)*@skku.edu)", $value)) return new Object(0, 'msg_unauth_email_address');
							if($member_srl && $logged_info->member_srl != $member_srl ) return new Object(0,'msg_exists_email_address');
						}
                        return new Object(0, 'allowed_email_address');
                    break;
				case 'opentime' :
						if (!ereg("^[[:digit:]]{2}[:][[:digit:]]{2}$", $value)) return new Object(0, 'not_allowed_format');
						return new Object(0, 'allowed_format');
				case 'closetime' :
						if (!ereg("^[[:digit:]]{2}[:][[:digit:]]{2}$", $value)) return new Object(0, 'not_allowed_format');
						return new Object(0, 'allowed_format');
            }
        ...
        function procMemberInsert() {
            ...
            // 필수 정보들을 미리 추출
            $args = Context::gets('user_id','user_name','nick_name','homepage','blog','birthday','email_address','password','allow_mailing','find_account_question','find_account_answer', 'skkumember','group_srl_list');
			...
			if ($args->group_srl_list == '237' || $args->group_srl_list == '1338'){
				if ($email_host != 'skku.edu' && $email_host != 'skku.ac.kr')
				return new Object(-1, 'skku.edu 이메일주소를 적어주세요.');
			}
        ...
        function procMemberResendAuthMail() {
			/*-----------------------------------
			* 11.3.25 이메일주소만으로인증메일 발송 -> 이메일주소를 잘못적어 인증불가능한것을 방지하기위해
			* 아이디와 비밀번호가 같으면 다른이메일로 인증메일을 발송하도록 수정
			* - 병웅
			*/
            ...
            $email_address = Context::get('email_address');
            $user_id = Context::get('user_id');
            $password = Context::get('user_passwd');
            
            if(!$user_id) return $this->stop('msg_invalid_request');
...
//            $member_info = $oMemberModel->getMemberSrlByEmailAddress($email_address);
			$member_info = $oMemberModel->getMemberSrlByUserID($user_id);
			...
			// 그룹검사후 성균관대생이면 이메일을 skku.edu로 받는다.
			$logged_info->group_list = $oMemberModel->getMemberGroups($member_info->member_srl);
			$group_val = array_values($logged_info->group_list);
            list($args->email_id, $args->email_host) = explode('@',$email_address);
			if ($group_val[0] == '성균관대'){
				if ($args->email_host != 'skku.edu' && $args->email_host != 'skku.ac.kr')
				return new Object(-1, 'skku.edu 이메일주소를 적어주세요.');
			}
			...
			if(!$args->member_srl) $args->member_srl = $member_info->member_srl;
        ...
        function setSessionInfo($member_info = null) {
            ...
            */
            ...
            $this->addMemberMenu( 'dispMemberOwnOrder', 'cmd_view_own_order');
			$this->addMemberMenu( 'dispMemberSalesGraph', 'cmd_view_sales_graph');
            $this->addMemberMenu( 'dispMemberOrderedList', 'cmd_view_ordered_list');
            $this->addMemberMenu( 'dispMemberRecentOrderedList', 'cmd_view_recent_ordered_list');
        }
        ...
        function insertMember(&$args, $password_is_hashed = false) {
            ...
            $member_srl = $oMemberModel->getMemberSrlByNickName($args->user_id2011-02-04);
        ...
        function updateMember($args) {
			...
			// email address 도메인을 skku.edu로 고정시킴
/*
			$args->email_host = strtolower($args->email_host);   
			if($args->email_host != 'skku.edu') return new Object(-1,'msg_uks');
*/
