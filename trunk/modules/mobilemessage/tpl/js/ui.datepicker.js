...
function Datepicker() {
	...
	this.regional['ko'] = { // Default regional settings
		clearText: '지움', // Display text for clear link
		clearStatus: 'Erase the current date', // Status text for clear link
		...
		dateFormat: 'yy-mm-dd', // See format options on parseDate
		firstDay: 0, // The first day of the week, Sun = 0, Mon = 1, ...
		initStatus: '날짜를 선택해 주세요', // Initial Status text on opening
		isRTL: false // True if right-to-left language, false if left-to-right
	};
		...
		for (var row = 0; row < numMonths[0]; row++)
			for (var col = 0; col < numMonths[1]; col++) {
				...
				for (var dow = 0; dow < 7; dow++) { // days of the week
					...
					html += '<td' + ((dow + firstDay + 6) % 7 >= 6 ? ' class="ui-datepicker-week-end-cell"' : '') + '>' +
						(!changeFirstDay ? '<span' :
				...
				for (var dRow = 0; dRow < numRows; dRow++) { // create date picker rows
					...
					for (var dow = 0; dow < 7; dow++) { // create date picker days
						...
						html += '<td class="ui-datepicker-days-cell' +
							...
							((dow + firstDay + 6) % 7 == 6 ? ' ui-datepicker-week-sun-cell' : '') + // highlight weekends
