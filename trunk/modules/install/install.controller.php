<?php
    ...
    class installController extends install {
        ...
        function procInstall() {
            ...
            $yamnyam_info = Context::gets('first_page', 'xe_path');
            ...
            // DB 복구
            /*
exec("mysql -u ".$db_info->db_userid." -p".$db_info->db_password." ".$db_info->db_database.' < '.$yamnyam_info->xe_path."db.sql");
            return new Object(-1, "mysql -u ".$db_info->db_userid." -p".$db_info->db_password." ".$db_info->db_database." < ".$yamnyam_info->xe_path."db.sql");
*/
/*
			$fp = fopen($yamnyam_info->xe_path."test.sql", "r");
			$test_data = fread($fp, 800);
			
			$oDBq = &DB::getInstance();
			$queryq = $oDBq->_query("SOURCE ".$yamnyam_info->xe_path."test.sql");
			$resultq = $oDBq->_fetch($queryq);
			
			
			return new Object(-1, $resultq);
*/
	//////////
	$connect = mysql_connect($db_info->db_hostname, $db_info->db_userid, $db_info->db_password);
	
	mysql_select_db($db_info->db_database, $connect);

	$file_content = file($yamnyam_info->xe_path."/start_db/yamnyam_db.sql");
	$query = "";
	...
	///////// start page
	mysql_query("update yman_sites set index_module_srl=".$yamnyam_info->first_page." where site_srl=0;");
