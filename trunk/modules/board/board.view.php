    ...
    class boardView extends board {
        ...
        function dispBoardCategoryList(){
            ...
            if($this->module_info->use_category=='Y') {
				...
				Context::set('restaurant_category_list', $oDocumentModel->getCategoryList(141));
            }
        }
...
//            return new Object();
        ...
        function dispBoardContentList(){
            ...
            $args->search_min = Context::get('search_min');
            $args->search_max = Context::get('search_max');
        ...
        function dispOrderConfirm() {
			...
			/*******
			* 11.3.30 최근 배송지 출력을위한 코드추가 - 병웅
			*******/
			Context::set('recent_address',$_SESSION['recent_address']);
			Context::set('voted_document',$_SESSION['voted_document']);
            $this->setTemplateFile('order_confirm');
        }
        /**
         * @brief 주문 작성 화면 출력(모바일)
         **/
        function dispOrderConfirmMobile() {
            // 권한 체크
            if(!$this->grant->write_document) return $this->dispBoardMessage('msg_not_permitted');
			...
			*******/
			...
			$data_str = Context::get('orderData');

			if($data_str){
				$data_temp = explode("|",$data_str);
	
				for($i=0; $i<count($data_temp)-1; $i++ ){
				  	$data= explode(":",$data_temp[$i]);
				  	$document_srls[$i] = $data[0];
				  	$amount[$data[0]]=$data[1];
			  	}
			  	$oDocument = $oDocumentModel->getDocuments($document_srls);
			  	Context::set('Document_list',$oDocument);
