<?php
    ...
    class documentAdminController extends document {
         ...
         * @brief 이벤트가격 변경
         **/
        function procDocumentAdminEditEventPrice() {
            $d_srl = Context::get("d_list");
            $d_srl = explode("|@|",$d_srl);
            
        	$oDB = &DB::getInstance();
			
        	foreach ($d_srl as $document_srl){
        		$price = Context::get("d_".$document_srl);
        		if($price == "" || $price == null){
        			$price = Context::get("o_".$document_srl);
        			$result = $oDB->_query("update yman_document_extra_vars set value='".$price."' where document_srl=".$document_srl." and var_idx=1");
			    	$output = $oDB->_fetch($result);
			    	$result = $oDB->_query("update yman_document_extra_vars set value='' where document_srl=".$document_srl." and var_idx=10");
			    	$output = $oDB->_fetch($result);
        		}
        		else {
        			$price = Context::get("d_".$document_srl);
			    	...
			    	$output = $oDB->_fetch($result);
			    	$price = Context::get("o_".$document_srl);
			    	$result = $oDB->_query("update yman_document_extra_vars set value='".$price."' where document_srl=".$document_srl." and var_idx=10");
			    	$output = $oDB->_fetch($result);
        		}
            }
