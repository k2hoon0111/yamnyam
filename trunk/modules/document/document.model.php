<?php
    ...
    class documentModel extends document {
        ...
        function getDocumentList($obj, $except_notice = false, $load_extra_vars=true) {
			...
			if($obj->module_srl==55 && !$search_target && $obj->adminpage!="yes") {
                $args->s_search_min = 0;
                $args->s_search_max = 30000;
                ...
                $args->s_current_time = $current_time_str;
                                
                if($search_keyword) $search_keyword = str_replace(' ','%',$search_keyword);
                $args->s_title = '%'.$search_keyword.'%'; 
                $use_division = true;
         	    $args->s_var_idx = '1';
                $args->s_var_value = 'price';
				$args->s_var_idx1 = '9';
                ...
                // 새로운방식의 랜덤 10분마다 바뀐다.
                $args->sort_index = 'rand(round(date_format( now() , "%i"), -1))';

                $query_id = 'document.getDocumentListWithPrice';
			}
			else {
	            $search_min = $obj->search_min;
	            $search_max = $obj->search_max;
				$current_time = $obj->current_time;
				
			}
			if($action == "dispDocumentAdminEventList") $query_id = 'document.getDocumentListEventList';
			if($obj->module_srl==141) {
            ...
            if($search_target) {
                switch($search_target) {
                    ...
                    case 'tag' :
                        ...
                        break;
                    case 'price' :
                            $args->s_search_min = $search_min;
                            $args->s_search_max = $search_max;
