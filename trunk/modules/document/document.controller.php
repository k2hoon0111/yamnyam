<?php
    ...
    class documentController extends document {
		...
		function procDocumentGradeUp() {
            ...
            $point = 1;
            return $this->updateVotedCount($document_srl, $point, $grade);
        }
        /**
        ...
        function insertDocument($obj, $manual_inserted = false) {
            ...
            if(Context::get('is_logged')&&!$manual_inserted) {
                ...
                // yamnyam Admin 음식추가를위한기능
                if($logged_info->user_id == "admin" && $obj->mid=="food_list"){
                	$oMemberModel = &getModel('member');
                	$logged_info = $oMemberModel->getMemberInfoByMemberSrl($oMemberModel->getMemberSrlByNickName($obj->nick_name));
                }
         ...
         **/
        function updateVotedCount($document_srl, $point = 1, $grade=0) {
            if($point > 0) $failed_voted = 'failed_voted';
            else $failed_voted = 'failed_blamed';
			
            // 세션 정보에 추천 정보가 있으면 중단
            if($_SESSION['voted_document'][$document_srl]) return new Object(-1, '이미 추천하셨습니다.');
            ...
            else
            {
                ...
                //평점을 위한코드
                if($grade){
                	$args->value = $oDocument->getExtraValue(8)+$grade;
                	$args->document_srl = $document_srl;
                	$args->var_idx = 8;
                	$output = executeQuery('document.updateVotedGrade', $args);
                }
            }
            if(!$output->toBool()) return $output;
